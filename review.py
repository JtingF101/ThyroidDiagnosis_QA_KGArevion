...
class Review(nn.Module):
    ...
    def forward(
        ...
        input_embeds = torch.cat((kg_embeds_lm, token_embeds), dim=1) #add 3 tokens
...
class PretrainKGEmbedding(nn.Module):
    ...
    def forward(self, triple_ids):
        ...
        if triple_ids.shape[1] == 3:
            ...
            prefix = self.adapter(pretrain_embs).reshape(-1, 3*self.num_prefix, self.llm_dim)  ##Here, num_prefix corresponds to the number of tokens associated with each entity/relationship.
            return prefix
