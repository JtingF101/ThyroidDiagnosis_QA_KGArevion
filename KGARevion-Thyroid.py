...
from action.retrieve import Retrieve
...
class KGARevion(object):
    def __init__(self,
        ...
        self.retriever = Retrieve(self.llm, args)
    ...
    def call(self, query):
        ...
        # result = self.triplets_generator.call(query)
        ...
        #  Supporting Literature (Optional)
        related_literature = self.retriever.call(filtered_triplets, query)
        print("Related Literature:", related_literature)
        logging.info("Related Literature: %s", related_literature)
        return answer
...
def main(args):
    ...
    if args.dataset in ['MedDDx', 'MedDDx-Basic', 'MedDDx-Intermediate', 'MedDDx-Expert']:
        ...
        # sampled_indices = random.sample(range(len(data)), 50)
        # Sampling Data
    ...
    with open("results/" + args.dataset + "_" + args.llm_name + "_is_Revision_" + str(args.is_revise) + "_round_x" + str(args.max_round) + "_all.txt", 'w') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", default='MedDDx', choices=['mmlu', 'medqa', 'pubmedqa', 'bioasq', 'MedDDx', 'MedDDx-Basic', 'MedDDx-Intermediate', 'MedDDx-Expert', 'afrimedqa_v2','thyroid','AfrimedQA-SAQ'], type=str)
    ...
    parser.add_argument("--weights_path", type=str, default='thyroid-fintune/')
