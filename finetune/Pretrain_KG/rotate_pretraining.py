...
class LoadPrimeKG(object):
    ...
    def read_entity_dict(self, entity_index, entity_name, rel):
        ...
        print("KG has {} entities.".format(len(entity_dict)))
...
set_seed(42)
data = LoadPrimeKG(data_path="pre_train_Thyroid_KG/")
...
# primeKG_dataloader = DataLoader(primeKG_dataset, batch_size = 128, shuffle=True, num_workers=2, pin_memory=True, drop_last=True, collate_fn=my_collate)
...
for epoch in range(num_epochs):
    ...
    if epoch > 4 and (total_loss/len(epoch_iterator)) < min_loss:
        ...
        stop_sign = 0
        torch.save(model.state_dict(), 'Thyroid_rotate_new.pth')
        print("save model!")
