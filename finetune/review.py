...
class Review(nn.Module):
    def __init__(
        ...
        self.llama_model = model
        self.max_length = 256
        ##for 8B 4096, while for 70B it shold be 8192

        self.device = self.llama_model.device
        self.lm_to_kg = LmToKG(num_layers = 1, hidden_dim = hidden_dim, embed_dim = 4096, device=self.device)
    ...
    def forward(
        ...
        input_embeds = torch.cat((kg_embeds_lm, token_embeds), dim=1)
